...
/*                                                +#+#+#+#+#+   +#+           */
/*   Created: 2025/08/14 01:32:49 by mwallis           #+#    #+#             */
/*   Updated: 2025/08/14 02:45:42 by mwallis          ###   ########.fr       */
/*                                                                            */
/* ************************************************************************** */
...
# define MOUSE_SENSIBILITY 0.0002
# define MOUSE_MAX_DELTA 400
...
typedef struct s_raycast_data
{
	...
	int		ray_current_tile_x;
	int		ray_current_tile_y;
	double	dist_to_next_x;
	double	dist_to_next_y;
	double	dist_per_tile_x;
	double	dist_per_tile_y;
	int		advance_direction_x;
	int		advance_direction_y;
	int		hit_side;
...
Ensure MLX is properly initialized and sufficient memory is \
available.\033[0m\n"
...
void		get_south_texture_path(char *line, t_map_data *map_data, \
									int fd, char *line_origin);
void		get_west_texture_path(char *line, t_map_data *map_data, \
									int fd, char *line_origin);
...
void		raycast_digital_differential_analyzer(const t_resources *resources, \
						t_raycast_data *ray);
mlx_image_t	*determine_texture_and_hit_column(const t_resources *resources, \
...
void		move_player_linear(t_player_data *player_data, \
								t_map_data *map_data, \
								double linear_speed, int direction);
void		move_player_lateral(t_player_data *player_data, \
								t_map_data *map_data, \
								double lateral_speed, int direction);
void		attempt_player_move(t_player_data *player_data, \
								t_map_data *map_data, \
